...
class _HomePageState extends State<HomePage> {
  ...
  Widget? _buildFab(String title) {
    ...
    // زر إنشاء طلب باسم زبون للمدير والمحاسب
    if ((widget.session['role'] == 'admin' || widget.session['role'] == 'accountant') && 
        (title == 'الطلبات' || title == 'قائمة الطلبات الجديدة')) {
      return FloatingActionButton.extended(
        onPressed: () => _showCreateOrderForCustomerDialog(),
        label: const Text('إنشاء طلب لزبون'),
  ...
  void _showCreateOrderForCustomerDialog() {
    // جلب قائمة الزبائن أولاً
    showDialog(
      context: context,
      builder: (context) => FutureBuilder<Map<String, dynamic>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            return AlertDialog(
              content: Text('خطأ: ${snapshot.error}'),
              actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('إغلاق'))],
            );
          }
          
          final customers = (snapshot.data?['customers'] ?? []) as List<dynamic>;
          
          return AlertDialog(
            title: const Text('اختر الزبون لإنشاء الطلب'),
            content: SizedBox(
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  return ListTile(
                    ...
                    onTap: () {
                      Navigator.pop(context);
                      // الانتقال لصفحة إنشاء الطلب مع تمرير بيانات الزبون
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                          builder: (_) => NewOrderScreen(
                            session: {
                              ...widget.session,
                              'selected_customer_id': customer['customer_id'] ?? customer['id'],
                              'selected_customer_name': customer['full_name'] ?? customer['name'],
                            },
                          ),
                        ),
                      );
